import { router } from "@inertiajs/react";
import React from "react";
import { CopyPlus } from "lucide-react";

const Index = () => {
    const handleCreateNewPost = () => {
        router.visit("/admin/posts/create");
    };

    return (
        <div className="min-h-screen bg-gray-900 p-6">
            {/* Dashboard Header */}
            <header className="mb-6 flex justify-between items-center mt-14">
                <div className="space-y-2">
                    <h1 className="text-3xl font-bold text-white">Dashboard</h1>
                    <p className="text-gray-300">
                        Welcome back! Here's a summary of your posts.
                    </p>
                </div>

                <button
                    onClick={handleCreateNewPost}
                    type="button"
                    className="flex gap-2 text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-full text-sm px-3 py-2 hover:cursor-pointer transition-colors duration-150 ease-in-out"
                >
                    <CopyPlus size={24} />
                    <span>Create New Post</span>
                </button>
            </header>

            {/* Table Card */}
            <div className="bg-gray-800 rounded-2xl shadow p-4">
                <div className="overflow-x-auto rounded-md">
                    <table className="w-full text-sm text-left text-gray-300">
                        <thead className="text-xs uppercase bg-gray-700 text-gray-200">
                            <tr>
                                <th className="px-6 py-3">Title</th>
                                <th className="px-6 py-3">Content</th>
                                <th className="px-6 py-3">Author</th>
                                <th className="px-6 py-3">Image_Url</th>
                                <th className="px-6 py-3 text-left w-[100px]">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            {[...Array(3)].map((_, index) => {
                                const items = [
                                    {
                                        title: 'Apple MacBook Pro 17"',
                                        content: "Silver",
                                        author: "Laptop",
                                        imageUrl: "$2999",
                                    },
                                    {
                                        title: "Microsoft Surface Pro",
                                        content: "White",
                                        author: "Laptop PC",
                                        imageUrl: "$1999",
                                    },
                                    {
                                        title: "Magic Mouse 2",
                                        content: "Black",
                                        author: "Accessories",
                                        imageUrl: "$99",
                                    },
                                ];

                                const item = items[index];

                                return (
                                    <tr
                                        key={index}
                                        className="bg-gray-900 border-b border-gray-700 hover:bg-gray-800"
                                    >
                                        <th className="px-6 py-4 font-medium text-white whitespace-nowrap">
                                            {item.title}
                                        </th>
                                        <td className="px-6 py-4">
                                            {item.content}
                                        </td>
                                        <td className="px-6 py-4">
                                            {item.author}
                                        </td>
                                        <td className="px-6 py-4">
                                            {item.imageUrl}
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end space-x-4">
                                                <a
                                                    href="#"
                                                    className="text-sm font-medium text-blue-400 hover:underline"
                                                >
                                                    Edit
                                                </a>
                                                <a
                                                    href="#"
                                                    className="text-sm font-medium text-red-400 hover:underline"
                                                >
                                                    Remove
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                                );
                            })}
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    );
};

export default Index;
